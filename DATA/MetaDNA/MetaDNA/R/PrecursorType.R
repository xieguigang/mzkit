#Region "Microsoft.ROpen::4ea68f9032b22830361fab87895ad15f, PrecursorType.R"

    # Summaries:

    # PrecursorType <- function() {#' Calculate m/z#'#' @param mass Molecule weight#' @param adduct adduct mass#' @param charge precursor charge value#'#' @return Returns the m/z value of the precursor ionadduct.mz <- function(mass, adduct, charge) {...
    ...
    # get.mass <- function(chargeMode, charge, PrecursorType) {if (PrecursorType %in% c("[M]+", "[M]-")) {...
    # get.PrecursorMZ <- function(M, precursorType) {...
    # find.PrecursorType <- function(mass, precursorMZ, charge, chargeMode = "+", minError.ppm = 100, debug.echo = TRUE) {if (charge == 0) {print("I can't calculate the ionization mode for no charge(charge = 0)!")}if (is.null(mass) || is.na(mass) || is.null(precursorMZ) || is.na(precursorMZ)) {if(is.null(mass)) {...
