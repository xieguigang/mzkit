...
#'
#' @param KEGG A list object with data structure like:
#'             \code{DB[KEGGID] = [KEGGID, mass, formula, name]}
#' @param precursor The MS1 information, \code{m/z}
#' @param tolerance The tolerance predicate method, by default is a method with ppm 20 precision.
#' @param precursor_type The \code{precursor_type} of this \code{MS1} value.
#'
#' @return Returns one of the element in list \code{KEGG}
...
#' The default SSM \code{ms/ms} similarity method
#'
#' @param threshold The SSM score threshold, by default is greater than 0.8
#'
#' @return returns a lambda function that using for predicate is query ms2 matrix
#'         similar to the subject ms2 matrix.
ms2.similar.fn <- function(threshold = 0.8) {
    function(q, s) {
        q   <- q[, 2];
		s   <- s[, 2];
