...
let source_dir = "\\192.168.1.254\backup3\项目以外内容\human_reference_metabolome\benchmark\MTBLS6039\FILES\RAW_FILES\NEG\mzPack";
let rawfiles = list.files(source_dir, pattern = "*.mzPack");

print(basename(rawfiles ));

let peaks = read.xcms_peaks("\\192.168.1.254\backup3\项目以外内容\human_reference_metabolome\benchmark\MTBLS6039\FILES\RAW_FILES\NEG\mzXML\peak_tablenew_nofill.txt", 
    tsv = TRUE);
...
write.csv(as.data.frame(assigned), file = "\\192.168.1.254\backup3\项目以外内容\human_reference_metabolome\benchmark\MTBLS6039\FILES\RAW_FILES\NEG\mzPack\test.csv");

assigned
|> JSON::json_encode()
|> writeLines(con = "\\192.168.1.254\backup3\项目以外内容\human_reference_metabolome\benchmark\MTBLS6039\FILES\RAW_FILES\NEG\mzPack\test.json")
;

assigned = unpack_assign(assigned);

for(let name in names(assigned)) {
    write.cache(assigned[[name]], file = `\\192.168.1.254\backup3\项目以外内容\human_reference_metabolome\benchmark\MTBLS6039\FILES\RAW_FILES\NEG\mzPack\cache/${name}.cache`);
}

for(let name in names(unassigned)) {
    write.cache(unassigned[[name]], file = `\\192.168.1.254\backup3\项目以外内容\human_reference_metabolome\benchmark\MTBLS6039\FILES\RAW_FILES\NEG\mzPack\unmapped/${name}.cache`);
}