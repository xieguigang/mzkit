 ...
 *
 * 0: Global variables
 * 1: Scaling of column widths
 * 2: Line Series prototype
 * 3: setExtremes
 * 4: x positioning of columns
 * 5: add ticks
...
// 		neutral = chart.neutral = new Neutral(chart);
...
// 			x1 = x2 = translate(pos + tickmarkOffset) + transB;
		...
		function setLinearTickPositions() {
			...
			if (isXAxis) {
				tickLength = 583/(tickPositions.length - 1);
				xAxisLength = tickPositions[tickPositions.length - 1] - tickPositions[0];
				tickMin = tickPositions[0];
			}
	...
	function MouseTracker (chart, options) {
		...
		*/
		...
		function setDOMEvents () {
			...
			imagemap.ondblclick = function(e) {
			e = normalizeMouseEvent(e);
				if (zoomed = true){
					fireEvent(chart, 'selection', { resetSelection: true }, zoom);
					chart.toolbar.remove('zoom');
			...
			imagemap.onmousemove = function(e) {
				...
				if (mouseIsDown) { // make selection
					...
					if (zoomY) {
						...
						setStyles(selectionMarker, {
							height: 1000 - ySize + PX,
							top: ((ySize > 0 ? ySize : ySize) +
		...
		if (chart.options.tooltip.neutral == true)
			tooltip.refresh(1);
	...
	var Legend = function(chart) {
		...
		for (var i = 0, max = allItems.length; i < max; i++) {
			if (i != options.startNumber)
				allItems[i].setVisible2();
		}
		...
		function renderHTML(clear) {
			...
			if(options.showFragments)
				str = '<left><b>(Mode) &nbsp;&nbsp;&nbsp; CollE &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Prec. Ion </left></b>';
			else if(options.showPeaks)
				str = '<left><b>(Mode) &nbsp;&nbsp;&nbsp; CollE &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Prec. Ion </left></b>';
			else if(options.showNeutrals)
				str = '<left><b>(Mode) &nbsp;&nbsp;&nbsp; CollE &nbsp;&nbsp;&nbsp; </left></b>';
	
			legendtitle = li = createElement('li', {
				innerHTML: str
// 				className: 'highchart-legend'
				},
// 				null, //item.visible ? style : merge(style, options.itemHiddenStyle), 
				dom.firstChild
			);
			addEvent(li, 'click', function() {
				fireEvent (legendtitle, 'legendItemClick', null, function() {
					if(isOn) {
						turnoff();
						isOn = false;
					}
...
// 	var Neutral = function(chart) {
// 		// already existing			
// 		//if (chart.legend) return;
// 			
// 		var options = chart.options.neutral;
// 			
// 		if (!options.enabled) return;
// 		
// 		var li,
...
// 			topRule = '#'+ container.id +' .highcharts-legend li', // apply once for each chart
// 			allItems = [],
// 			neutralLayer = new Layer('legend', container, null, { zIndex: 8 }),
// 			neutralArea,
// 			isOn = false,
// 			neutraltitle,
// 			str;
...
// 	};
	...
	function Tooltip (options) {
		...
		if(options.second || options.neutral) {
			var innerDiv2,
			boxLayer2;
		}
		...
		if(options.second || options.neutral) {
			...
			tooltipDiv2 = createElement(DIV, null, {
				position: ABSOLUTE,
				visibility: HIDDEN,
				overflow: HIDDEN,
				padding: '0 50px 5px 0',
			...
			boxLayer2 = new Layer('tooltip-box', tooltipDiv2, null, {
				width: plotWidth + PX,
				height: plotHeight + PX
			});

			innerDiv2 = createElement(DIV, { 
					className: "highcharts-tooltip"
				}, extend(options.style, {
					maxWidth: (chartWidth - 40) + PX,
					//overflow: HIDDEN, interferes with the text width detection
		...
		function refresh(point, series) {
			if (point != 1) {
				...
				if(options.second) {
					...
					boxWidth2,
					boxHeight2,
					oldInnerDivHeight2 = innerDiv2.offsetHeight,
					text2 = point.tooltipText2;
				}
				...
				if (x >= 0 && x <= plotWidth && y <= plotHeight) {
					show = true;
				}
...
// 				document.writeln(text2);
					if(text2 === false || !show)
						...
						innerDiv2.innerHTML = text2;
						setStyles(innerDiv2, { overflow: VISIBLE });
						boxWidth2 = innerDiv2.offsetWidth - borderWidth;
						boxHeight2 = innerDiv2.offsetHeight - borderWidth;
						setStyles(innerDiv2, { overflow: HIDDEN });
						if (boxWidth2 > (boxLayer2.w || 0) + 20 || boxWidth2 < (boxLayer2.w || 0) - 20 || 
								boxHeight2 > boxLayer2.h || boxLayer2.c != borderColor ||
								oldInnerDivHeight2 != innerDiv2.offsetHeight ) {
						boxLayer2.clear();		
						boxLayer2.drawRect(
							borderWidth / 2, 
						...
						// keep the box within the chart area
						boxX2 = x - boxLayer2.w + marginLeft - 35;
						boxY2 = y - boxLayer2.h + 10 + marginTop;
						
						// it is too far to the left, and there is space to the right
						if ((inverted || boxX2 < 5) && x + marginLeft + boxLayer2.w < chartWidth - 100) 
							boxX2 = x + marginLeft + 15; // right align
						...
						if (boxY2 < 5) boxY2 = 5; // above
						else if (boxY2 + boxLayer2.h > chartHeight) 
							boxY2 = chartHeight - boxLayer2.h - 5; // below
						
						boxX2 = 310; boxY2 = 300;
		
						// do the move
						move2(mathRound(boxX2), mathRound(boxY2));
				
						// show the hover mark
						// series.drawPointState(point, 'hover');
					
						tooltipDiv2.style.visibility = VISIBLE;
					}
				}
			}
			
			else if (point == 1) {
				var borderColor = options.borderColor || '#606060',
				boxX2,
				boxY2,
				boxWidth2,
				boxHeight2,
				...
				mass1 = (selectedPoints[0] == false) ? '<b> Left Click to Select </b>' : '<b><font size="3" color=" ' + selectedPoints[0].series.color + '">' + (selectedPoints[0].x).toFixed(4) + '</font></b>';
				mass2 = (BrowserDetect.OS == 'Mac') ? (selectedPoints[1] == false) ? '<b> Cmd + Left Click to Select </b>' : '<b><font size="3" color=" ' + selectedPoints[1].series.color + '">' + (selectedPoints[1].x).toFixed(4) + '</font></b>' : (selectedPoints[1] == false) ? '<b> Ctrl + Left Click to Select </b>' : '<b><font size="3" color=" ' + selectedPoints[1].series.color + '">' + (selectedPoints[1].x).toFixed(4) + '</font></b>';
				lossnum = (selectedPoints[0].x - selectedPoints[1].x >= 0) ? (selectedPoints[0].x - selectedPoints[1].x).toFixed(4) : (selectedPoints[1].x - selectedPoints[0].x).toFixed(4);
				lossform = ((lossnum < 16) ? '</font></b> <form action="javascript:alert(\'Must search m/z values greater than 16\')">' : '</font></b> <form action="../Fragment/fragment_list.php" method="post">') + '<input type="hidden" name="frag_mid" value="' + lossnum + '"> <input type="hidden" name="frag_mode" value="1"> &nbsp;&nbsp; Tolerance (&plusmn): <input align="right" type="text" name="frag_tol" size="4" value="100"> ppm &nbsp; <select name="frag_chrg"> <option value = "1">Positive</option> <option value = "-1">Negative</option> </select> ';
				lossform += ((lossnum < 16) ? '<input type="submit" value="Search"></form>' : '<input type="submit" value="Search" onclick="this.form.target=\'_blank\';return true;"></form>') ;
				loss = (selectedPoints[0] == false || selectedPoints[1] == false) ? '<b> Select two points first! </b>' : (selectedPoints[0].series == selectedPoints[1].series) ? '<b><font size="3">' + lossnum + '</font></b>' + lossform : '</font><b> Points must the same series! </b><font>';
				text2 = '<b><center><font size="3"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Neutral Loss Calculator  </center></font></b><br/> &nbsp;&nbsp; Point 1 &nbsp;&nbsp; m/z: &nbsp;&nbsp;&nbsp;&nbsp; ' + mass1 + '<br/> &nbsp;&nbsp; Point 2 &nbsp;&nbsp; m/z: &nbsp;&nbsp;&nbsp;&nbsp; ' + mass2 + '<br/>';
				text2 += ' &nbsp;&nbsp; Neutral &nbsp; Loss: &nbsp;&nbsp; ' + loss + '<br/>';
		...
		if(options.second || options.neutral) {
			function move2(finalX2, finalY2) {
				
				var hidden = (tooltipDiv2.style.visibility == HIDDEN),
					x = hidden ? finalX2 : (tooltipDiv2.offsetLeft + finalX2) / 2, 
					y = hidden ? finalY2 : (tooltipDiv2.offsetTop + finalY2) / 2;
				
				setStyles( tooltipDiv2, {
					left: x + PX, 
					top: y + PX
				});
				
				// run on next tick of the mouse tracker
				if (mathAbs(finalX2 - x) > 1 || mathAbs(finalY2 - y) > 1) {
					tooltipTick2 = function() {
						move2(finalX2, finalY2);
					};
				} else {
					tooltipTick2 = null;
				}
		...
		function hide(tooltipDivNum) {
			if (tooltipDivNum == 1) tooltipDiv.style.visibility = HIDDEN;
			if (options.second) 
				if (tooltipDivNum == 2) tooltipDiv2.style.visibility = HIDDEN;
	...
	select: function(selected, accumulate) {
		...
		if (!accumulate) {
			if(!point.right) {
				...
				point.left = (point.left == true) ? false: true;
				selectedPoints[0] = (selectedPoints[0] != point) ? point : false;
				series.isDirty = true;
			}
			else {
				point.right = false;
			...
			if(!point.left) {
				point.selected = selected = pick(selected, !point.selected);
				point.right = (point.right == true) ? false : true;
				selectedPoints[1] = (selectedPoints[1] != point) ? point : false;
				series.isDirty = true;
			}
			else {
				point.left = false;
		...
		each (chart.series, function (series) {
			...
			if (!accumulate) each (series.data, function(loopPoint) {
				if (loopPoint.selected && loopPoint != point && loopPoint.left == true) {
					loopPoint.selected = false;
					loopPoint.left = false;
					fireEvent(loopPoint, 'unselect');
					series.isDirty = true;
				}
			});

			else if (accumulate) each (series.data, function(loopPoint) {
				if (loopPoint.selected && loopPoint != point && loopPoint.right == true) {
					loopPoint.selected = false;
					loopPoint.right = false;
					fireEvent(loopPoint, 'unselect');
					series.isDirty = true;
				}
			});
	...
	setTooltipPoints: function (renew) {
		...
		each (data, function(point, i) {
			...
			if (!series.tooltipPoints) {// only create the text the first time, not on zoom
				...
				if (chart.options.tooltip.second) {
					point.tooltipText2 = chart.options.tooltip.formatter2.call({
						series: series,
						point: point,
						x: point.category, 
						y: point.y,
...
var ColumnSeries = extendClass(Series, {
	...
	translate: function() {
		...
		each (data, function(point) {
			point.plotX = 3+583*((point.x-tickMin)/xAxisLength);
			point.w = pointWidth * Math.log((2.718))/Math.log(4);
	...
	drawPoints: function(state) {
		...
		each (data, function(point) {
			if (point.plotY !== undefined) layer.drawRect(
				...
				options.borderWidth, 
				options.borderRadius, 
				point.fragment ? '#000000' : point.color || series.color,
				options.shadow
			);
	...
	drawPointState: function(point, state, layer) {
		...
		if (state && this.options.states[state]) {
			...
			layer.drawRect(
				...
				options.borderWidth, 
				options.borderRadius, 
				point.selected || point.fragment ? Color('#000000').brighten(bright).get() : Color(options.color || this.color).brighten(options.brightness).get(), 
				options.shadow		
			)
		}
	},
