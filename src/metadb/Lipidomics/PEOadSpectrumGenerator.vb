...
Namespace CompMs.Common.Lipidomics
    Public Class PEOadSpectrumGenerator
        ...
        Public Function Generate(ByVal lipid As Lipid, ByVal adduct As AdductIon, ByVal Optional molecule As IMoleculeProperty = Nothing) As IMSScanProperty Implements ILipidSpectrumGenerator.Generate
            Dim abundance = 30
            Dim nlMass = If(adduct.IonMode = IonMode.Positive, C2H8NO4P, 0.0)
            ...
            '"OAD17",
            '"OAD01+H"
            Dim oadId = If(adduct.IonMode = IonMode.Positive, New String() {"OAD01", "OAD02", "OAD03", "OAD04", "OAD14", "OAD15", "OAD16", "OAD17", "OAD12+O", "OAD12+O+H", "OAD01+H"}, New String() {"OAD01", "OAD02", "OAD03", "OAD04", "OAD14", "OAD15", "OAD15+O", "OAD16", "OAD12+O", "OAD12+O+H", "OAD12+O+2H"})
        ...
        Private Function GetPEOadSpectrum(ByVal lipid As Lipid, ByVal adduct As AdductIon) As SpectrumPeak()
            Dim spectrum = New List(Of SpectrumPeak)()
...
}, New SpectrumPeak(adduct.ConvertToMz(lipid.Mass - C2H8NO4P), 999R, "Precursor -C2H8NO4P") With {
...
.IsAbsolutelyRequiredFragmentForAnnotation = True
                ...
                If CSharpImpl.__Assign(Chains, TryCast(lipid.Chains, SeparatedChains)) IsNot Nothing Then
                    For Each chain As AcylChain In lipid.Chains.GetDeterminedChains()
                        spectrum.AddRange({New SpectrumPeak(adduct.ConvertToMz(lipid.Mass - chain.Mass + HydrogenMass), 50R, $"-{chain}") With {
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(adduct.ConvertToMz(lipid.Mass - chain.Mass + HydrogenMass - H2O), 40R, $"-{chain}-H2O") With {
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(adduct.ConvertToMz(chain.Mass - HydrogenMass), 20R, $"{chain} Acyl+") With {
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(adduct.ConvertToMz(chain.Mass), 5R, $"{chain} Acyl+ +H") With {
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(adduct.ConvertToMz(chain.Mass + C3H5O2), 20R, $"-{chain} +C3H5O2") With {
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(adduct.ConvertToMz(chain.Mass + C3H5O2 + HydrogenMass), 10R, $"-{chain} +C3H5O2+H") With {
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(adduct.ConvertToMz(chain.Mass + C2H2O), 30R, $"-{chain} +C2H2O") With {
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(adduct.ConvertToMz(chain.Mass + C2H2O + HydrogenMass), 10R, $"-{chain} +C2H2O+H") With {
.SpectrumComment = SpectrumComment.acylchain
}})
                    Next
                End If
...
.SpectrumComment = SpectrumComment.precursor
}, New SpectrumPeak(adduct.ConvertToMz(C2H8NO4P), 30R, "Header-") With {
.SpectrumComment = SpectrumComment.metaboliteclass
}})

                If CSharpImpl.__Assign(Chains, TryCast(lipid.Chains, SeparatedChains)) IsNot Nothing Then
                    For Each chain As AcylChain In lipid.Chains.GetDeterminedChains()
                        spectrum.AddRange({New SpectrumPeak(chain.Mass + OxygenMass + Electron, 30R, $"{chain} FA") With {
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(lipid.Mass - chain.Mass + Electron, 30R, $"-{chain}") With {
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(lipid.Mass - chain.Mass + Electron - H2O, 15R, $"-{chain}-H2O") With {
.SpectrumComment = SpectrumComment.acylchain
}})
                    Next
                End If
