...
Namespace CompMs.Common.Lipidomics
    Public Class CerNSd7SpectrumGenerator
        ...
        Public Function Generate(ByVal lipid As Lipid, ByVal adduct As AdductIon, ByVal Optional molecule As IMoleculeProperty = Nothing) As IMSScanProperty Implements ILipidSpectrumGenerator.Generate
            ...
            If CSharpImpl.__Assign(sphingo, TryCast(lipid.Chains.GetChainByPosition(1), SphingoChain)) IsNot Nothing Then
                ...
                spectrum.AddRange(GetSphingoDoubleBondSpectrum(lipid, sphingo, adduct, nlmass, 100R))
            End If
            ...
            If CSharpImpl.__Assign(acyl, TryCast(lipid.Chains.GetChainByPosition(2), AcylChain)) IsNot Nothing Then
                ...
                spectrum.AddRange(spectrumGenerator.GetAcylDoubleBondSpectrum(lipid, acyl, adduct, nlmass - sphD7MassBalance, 30R))
            End If
    ...
    .Charge = adduct.ChargeNumber
        ...
        Private Function GetCerNSd7Spectrum(ByVal lipid As ILipid, ByVal adduct As AdductIon) As SpectrumPeak()
            Dim lipidD7mass = lipid.Mass + sphD7MassBalance
            Dim spectrum = New List(Of SpectrumPeak) From {
    New SpectrumPeak(adduct.ConvertToMz(lipidD7mass), 999R, "Precursor") With {
        .SpectrumComment = SpectrumComment.precursor
    }
}
            If Equals(adduct.AdductIonName, "[M+Na]+") Then
                spectrum.AddRange({New SpectrumPeak(adduct.ConvertToMz(lipidD7mass) - CH3O, 150R, "Precursor-CH3O") With {
.SpectrumComment = SpectrumComment.metaboliteclass,
.IsAbsolutelyRequiredFragmentForAnnotation = True
}})
            Else
                spectrum.AddRange({New SpectrumPeak(adduct.ConvertToMz(lipidD7mass) - H2O, 200R, "Precursor-H2O") With {
.SpectrumComment = SpectrumComment.metaboliteclass,
.IsAbsolutelyRequiredFragmentForAnnotation = True
}, New SpectrumPeak(lipidD7mass + ProtonMass - CH6O2, 100R, "[M+H]+ -CH6O2") With {
.SpectrumComment = SpectrumComment.metaboliteclass
}})
                If Equals(adduct.AdductIonName, "[M+H]+") Then
                    spectrum.AddRange({New SpectrumPeak(adduct.ConvertToMz(lipidD7mass) - H2O * 2, 100R, "Precursor-2H2O") With {
.SpectrumComment = SpectrumComment.metaboliteclass
}})
                End If
            End If
...
.SpectrumComment = SpectrumComment.acylchain
            ...
            Return spectrum.ToArray()
        End Function
        Public Function GetSphingoDoubleBondSpectrum(ByVal lipid As ILipid, ByVal sphingo As SphingoChain, ByVal adduct As AdductIon, ByVal nlMass As Double, ByVal abundance As Double) As IEnumerable(Of SpectrumPeak)
        ...
        Private Function GetAcylSpectrum(ByVal lipid As ILipid, ByVal acyl As AcylChain, ByVal adduct As AdductIon) As SpectrumPeak()
            ...
            Dim spectrum = New List(Of SpectrumPeak)()
            If Equals(adduct.AdductIonName, "[M+Na]+") Then
                spectrum.AddRange({New SpectrumPeak(adduct.ConvertToMz(chainMass) + C2H3NO + HydrogenMass, 150R, "[FAA+C2H2O+H]+") With {
.SpectrumComment = SpectrumComment.acylchain
}})
            Else
