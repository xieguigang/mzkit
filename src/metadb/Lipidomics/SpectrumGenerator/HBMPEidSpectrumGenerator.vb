...
Imports BioNovoGene.BioDeep.Chemoinformatics.Formula.MS
    ...
    New SpectrumPeak(lipid.Mass + adductmass - H2O, 200R, "Precursor -H2O") With {
        .SpectrumComment = SpectrumComment.metaboliteclass,
        .IsAbsolutelyRequiredFragmentForAnnotation = True
    },
    New SpectrumPeak(adduct.ConvertToMz(lipid.Mass) / 2, 200R, "[Precursor]2+") With {
        .SpectrumComment = SpectrumComment.precursor
    },
    New SpectrumPeak((lipid.Mass + adductmass - H2O) / 2, 200R, "[Precursor -H2O]2+") With {
        .SpectrumComment = SpectrumComment.metaboliteclass,
        .IsAbsolutelyRequiredFragmentForAnnotation = True
    },
    New SpectrumPeak(C3H9O6P + adductmass, 50R, "C3H9O6P") With {
        .SpectrumComment = SpectrumComment.metaboliteclass
    },
    New SpectrumPeak(C3H9O6P - H2O + adductmass, 50R, "C3H9O6P - H2O") With {
        .SpectrumComment = SpectrumComment.metaboliteclass
    }
}
            If Equals(adduct.AdductIonName, "[M+NH4]+") Then
                spectrum.AddRange({New SpectrumPeak(lipid.Mass + ProtonMass, 200R, "[M+H]+") With {
.SpectrumComment = SpectrumComment.metaboliteclass
}, New SpectrumPeak((lipid.Mass + ProtonMass) / 2, 200R, "[M+2H]2+") With {
.SpectrumComment = SpectrumComment.metaboliteclass
}})
            End If
