...
Namespace CompMs.Common.Lipidomics
    Public Class BMPEidSpectrumGenerator
        ...
        Public Function Generate(ByVal lipid As Lipid, ByVal adduct As AdductIon, ByVal Optional molecule As IMoleculeProperty = Nothing) As IMSScanProperty Implements ILipidSpectrumGenerator.Generate
            ...
            If lipid.Description.Has(LipidDescription.Chain) Then
                ...
                spectrum.AddRange(EidSpecificSpectrum(lipid, adduct, 0.0, 200R))
            End If
    ...
    New SpectrumPeak(lipid.Mass - C3H9O6P + adductmass, 100R, "Precursor -C3H9O6P") With {
        .SpectrumComment = SpectrumComment.metaboliteclass
    },
    New SpectrumPeak(C3H9O6P - H2O + adductmass, 200R, "C3H9O6P - H2O") With {
        ...
        Private Function GetAcylLevelSpectrum(ByVal lipid As ILipid, ByVal acylChain As IChain, ByVal adduct As AdductIon) As SpectrumPeak()
            ...
            Dim spectrum = New List(Of SpectrumPeak) From {
    New SpectrumPeak(lipidMass - chainMass + adductmass, 50R, $"-{acylChain}")
 }
            If Equals(adduct.AdductIonName, "[M+Na]+") Then
                spectrum.AddRange({New SpectrumPeak(lipidMass - chainMass - HydrogenMass - OxygenMass + adductmass, 50R, $"-{acylChain}-OH") With {
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(lipidMass - chainMass - C3H6O2 + adductmass, 400R, $"-C3H6O2 -{acylChain}") With {
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(lipidMass - chainMass - C3H6O2 - H2O + adductmass, 50R, $"-C3H6O2 -{acylChain}-H2O") With {
.SpectrumComment = SpectrumComment.acylchain
}})
            Else
                spectrum.AddRange({New SpectrumPeak(lipidMass - chainMass - H2O + adductmass, 50R, $"-{acylChain}-H2O") With {
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(lipidMass - chainMass - C3H9O6P + adductmass, 400R, $"-C3H9O6P -{acylChain}") With {
.SpectrumComment = SpectrumComment.acylchain
