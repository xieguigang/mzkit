...
Public Class PIOadSpectrumGenerator
        ...
        Public Function Generate(ByVal lipid As Lipid, ByVal adduct As AdductIon, ByVal Optional molecule As IMoleculeProperty = Nothing) As IMSScanProperty Implements ILipidSpectrumGenerator.Generate
            Dim abundance = 30
            Dim nlMass = If(adduct.IonMode = IonMode.Positive, C6H13O9P + NH3, 0.0)
            ...
            '"OAD17",
            '"OAD01+H"
            Dim oadId = If(adduct.IonMode = IonMode.Positive, New String() {"OAD01", "OAD02", "OAD02+O", "OAD03", "OAD04", "OAD14", "OAD15", "OAD15+O", "OAD16", "OAD17", "OAD12+O", "OAD12+O+H", "OAD01+H"}, New String() {"OAD01", "OAD02", "OAD03", "OAD04", "OAD09", "OAD10", "OAD14", "OAD15", "OAD12+O", "OAD12+O+H", "OAD12+O+2H"})
        ...
        Private Function GetPIOadSpectrum(ByVal lipid As Lipid, ByVal adduct As AdductIon, ByVal nlMass As Double) As SpectrumPeak()
            Dim spectrum = New List(Of SpectrumPeak)()
...
}, New SpectrumPeak(adduct.ConvertToMz(lipid.Mass) - NH3, 100R, "[M+H]+") With {
...
.SpectrumComment = SpectrumComment.precursor
}, New SpectrumPeak(adduct.ConvertToMz(C6H13O9P - H2O), 30R, "Header-") With {
.SpectrumComment = SpectrumComment.metaboliteclass
}})
...
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(lipid.Mass - C6H10O5 - H2O - chain.Mass + Electron, 20R, $"-Header-{chain}") With {
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(lipid.Mass - chain.Mass - H2O + Electron, 20R, $"-Header-{chain}") With {
.SpectrumComment = SpectrumComment.acylchain
}})
                    Next
                End If
