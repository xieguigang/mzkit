...
Namespace CompMs.Common.Lipidomics
    Public Class PCd5SpectrumGenerator
        ...
        Public Function CanGenerate(ByVal lipid As ILipid, ByVal adduct As AdductIon) As Boolean Implements ILipidSpectrumGenerator.CanGenerate
            If lipid.LipidClass = LbmClass.PC_d5 Then
    ...
    .Charge = adduct.ChargeNumber
        ...
        Private Function GetPCSpectrum(ByVal lipid As ILipid, ByVal adduct As AdductIon) As SpectrumPeak()
            Dim spectrum = New List(Of SpectrumPeak) From {
    New SpectrumPeak(adduct.ConvertToMz(lipid.Mass), 999R, "Precursor") With {
        .SpectrumComment = SpectrumComment.precursor
    },
    New SpectrumPeak(adduct.ConvertToMz(C5H14NO4P), 300R, "Header") With {
    ...
    New SpectrumPeak(adduct.ConvertToMz(lipid.Mass - C5H14NO4P - HydrogenMass * 2), 25R, "Precursor-Header") With {
        .SpectrumComment = SpectrumComment.metaboliteclass
    },
    New SpectrumPeak(adduct.ConvertToMz(lipid.Mass) / 2, 150R, "[Precursor]2+") With {
        .SpectrumComment = SpectrumComment.metaboliteclass
            ...
            If Equals(adduct.AdductIonName, "[M+Na]+") Then
                spectrum.AddRange({New SpectrumPeak(adduct.ConvertToMz(lipid.Mass - C5H14NO4P), 300R, "Precursor-Header") With {
.SpectrumComment = SpectrumComment.metaboliteclass
}, New SpectrumPeak(adduct.ConvertToMz(lipid.Mass - C3H9N), 200R, "Precursor-C3H9N") With {
.SpectrumComment = SpectrumComment.metaboliteclass
}})
            End If
        ...
        Private Function GetAcylLevelSpectrum(ByVal lipid As ILipid, ByVal acylChain As IChain, ByVal adduct As AdductIon) As SpectrumPeak()
            ...
            Return {New SpectrumPeak(adduct.ConvertToMz(lipidMass - chainMass), 50R, $"-{acylChain}") With {
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(adduct.ConvertToMz(lipidMass - chainMass - H2O), 50R, $"-{acylChain}-O") With {
.SpectrumComment = SpectrumComment.acylchain
}}
        End Function
        ...
        Private Function GetAcylPositionSpectrum(ByVal lipid As ILipid, ByVal acylChain As IChain, ByVal adduct As AdductIon) As SpectrumPeak()
            ...
            Return {New SpectrumPeak(adduct.ConvertToMz(lipidMass - chainMass - OxygenMass - CD2), 50R, "-CD2(Sn1)") With {
.SpectrumComment = SpectrumComment.snposition
        ...
        Private Function GetAcylDoubleBondSpectrum(ByVal lipid As ILipid, ByVal acylChains As IEnumerable(Of AcylChain), ByVal adduct As AdductIon) As IEnumerable(Of SpectrumPeak)
            Return acylChains.SelectMany(Function(acylChain) spectrumGenerator.GetAcylDoubleBondSpectrum(lipid, acylChain, adduct, 0R, 10R))
        End Function
