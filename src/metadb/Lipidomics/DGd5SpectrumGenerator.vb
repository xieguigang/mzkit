...
'new SpectrumPeak(adduct.ConvertToMz(lipid.Mass)/2, 50d, $"[precursor]2+") { SpectrumComment = SpectrumComment.precursor },
            ...
            If Equals(adduct.AdductIonName, "[M+NH4]+") Then
                ...
                spectrum.AddRange({New SpectrumPeak(lipid.Mass + ProtonMass, 150R, "[M+H]+") With {
            .SpectrumComment = SpectrumComment.metaboliteclass
                }, New SpectrumPeak(lipid.Mass + ProtonMass - H2O, 150R, "[M+H]+ -H2O") With {
            .SpectrumComment = SpectrumComment.metaboliteclass
                             }})
            ElseIf Equals(adduct.AdductIonName, "[M+H]+") Then
                spectrum.AddRange({New SpectrumPeak(adduct.ConvertToMz(lipid.Mass) - D2O, 100R, "Precursor-D2O") With {
.SpectrumComment = SpectrumComment.metaboliteclass
}})
            End If
        ...
        Private Function GetAcylLevelSpectrum(ByVal lipid As ILipid, ByVal acylChain As IChain, ByVal adduct As AdductIon) As SpectrumPeak()
            ...
            If Equals(adduct.AdductIonName, "[M+Na]+") Then
                spectrum.AddRange({New SpectrumPeak(adduct.ConvertToMz(chainMass + H2O), 50R, $"{acylChain}+O") With {
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(lipidMass - chainMass - HydrogenMass * 2, 50R, $"-{acylChain}") With {
.SpectrumComment = SpectrumComment.acylchain
}, New SpectrumPeak(lipidMass - chainMass - HydrogenMass - OxygenMass, 200R, $"-{acylChain}-O") With {
.SpectrumComment = SpectrumComment.acylchain
}})
            Else
