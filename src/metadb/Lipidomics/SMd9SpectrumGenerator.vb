...
Namespace CompMs.Common.Lipidomics
    Public Class SMd9SpectrumGenerator
        ...
        Private Shared ReadOnly C3D9N As Double = {CarbonMass * 3, Hydrogen2Mass * 9, NitrogenMass}.Sum()
        ...
        Public Function CanGenerate(ByVal lipid As ILipid, ByVal adduct As AdductIon) As Boolean Implements ILipidSpectrumGenerator.CanGenerate
            If lipid.Chains.OxidizedCount = 3 Then
                Return False
            End If
            If lipid.LipidClass = LbmClass.SM_d9 Then
    ...
    New SpectrumPeak(adduct.ConvertToMz(C5H5D9NO4P), 500R, "Header") With {
        .SpectrumComment = SpectrumComment.metaboliteclass
    },
    New SpectrumPeak(adduct.ConvertToMz(C5H5D9NO4P + C2H3NO - OxygenMass), 150R, "Header+C2H3N") With {
        .SpectrumComment = SpectrumComment.metaboliteclass
    }
}
            If Equals(adduct.AdductIonName, "[M+Na]+") Then
                spectrum.AddRange({New SpectrumPeak(adduct.ConvertToMz(lipid.Mass) - C3D9N, 150R, "Precursor-C3D9N") With {
.SpectrumComment = SpectrumComment.metaboliteclass
}, New SpectrumPeak(adduct.ConvertToMz(lipid.Mass) - C5H5D9NO4P, 150R, "Precursor-Header") With {
.SpectrumComment = SpectrumComment.metaboliteclass
}})
            ElseIf Equals(adduct.AdductIonName, "[M+H]+") Then
                spectrum.AddRange({New SpectrumPeak(adduct.ConvertToMz(C5H5D9NO4P + C2H3NO + CarbonMass), 150R, "Header+C3H3NO") With {
.SpectrumComment = SpectrumComment.metaboliteclass
}}) ' need to consider
            End If
        ...
        Private Function GetSphingoSpectrum(ByVal lipid As ILipid, ByVal sphingo As SphingoChain, ByVal adduct As AdductIon) As SpectrumPeak()
            ...
            If Equals(adduct.AdductIonName, "[M+H]+") Then
                spectrum.AddRange({New SpectrumPeak(chainMass + ProtonMass - H2O * 2, 100R, "[sph+H]+ -Header -H2O") With {
...
'new SpectrumPeak(chainMass + MassDiffDictionary.ProtonMass - CH4O2, 100d, "[sph+H]+ -CH4O2"),
        ...
        Private Function GetAcylSpectrum(ByVal lipid As ILipid, ByVal acyl As AcylChain, ByVal adduct As AdductIon) As SpectrumPeak()
            ...
            Dim spectrum = New List(Of SpectrumPeak)() From {
    New SpectrumPeak(adduct.ConvertToMz(chainMass) + C2H2N, 200R, "[FAA+C2H+adduct]+") With {
        .SpectrumComment = SpectrumComment.acylchain
    },
    New SpectrumPeak(adduct.ConvertToMz(chainMass) + C5H5D9NO4P + C2H2N - HydrogenMass, 200R, "[FAA+C2H+Header+adduct]+") With {
        .SpectrumComment = SpectrumComment.acylchain
    }
}
            Return spectrum.ToArray()
