...
Namespace CompMs.Common.Lipidomics
    ...
    .Charge = adduct.ChargeNumber
        ...
        Private Function GetCESpectrum(ByVal lipid As ILipid, ByVal adduct As AdductIon) As SpectrumPeak()
    ...
    New SpectrumPeak(adduct.ConvertToMz(lipid.Mass), 999R, "Precursor") With {
        .SpectrumComment = SpectrumComment.precursor
            ...
            If Equals(adduct.AdductIonName, "[M+NH4]+") Then
                spectrum.AddRange({New SpectrumPeak(skelton + massBalanceD7 - H2O + ProtonMass, 500R, "skelton") With {
...
'new SpectrumPeak(lipid.Mass+MassDiffDictionary.ProtonMass , 50d, "[M+H]+"){ SpectrumComment = SpectrumComment.metaboliteclass },
            ...
            ElseIf Equals(adduct.AdductIonName, "[M+Na]+") Then
                spectrum.AddRange({New SpectrumPeak(skelton + massBalanceD7 - H2O, 500R, "skelton") With {
.SpectrumComment = SpectrumComment.metaboliteclass,
.IsAbsolutelyRequiredFragmentForAnnotation = True
}})
            End If
        ...
        End Function
        Private Function GetAcylLevelSpectrum(ByVal lipid As ILipid, ByVal acylChain As AcylChain, ByVal adduct As AdductIon) As SpectrumPeak()
            Dim chainMass = acylChain.Mass - HydrogenMass
            Dim spectrum = New List(Of SpectrumPeak)()
            If Equals(adduct.AdductIonName, "[M+Na]+") Then
                spectrum.AddRange({New SpectrumPeak(adduct.ConvertToMz(chainMass + H2O), 200R, $"{acylChain}+O") With {
