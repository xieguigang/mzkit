...
Namespace CompMs.Common.Lipidomics
    ...
    .Charge = adduct.ChargeNumber
        ...
        End Function
        Private Function GetCLSpectrum(ByVal lipid As ILipid, ByVal adduct As AdductIon) As SpectrumPeak()
    ...
    New SpectrumPeak(adduct.ConvertToMz(lipid.Mass - H2O), 999R, "Precursor -H2O") With {
    ...
    New SpectrumPeak(adduct.ConvertToMz(lipid.Mass - H2O) / 2, 150R, "[Precursor -H2O]2+") With {
        .SpectrumComment = SpectrumComment.metaboliteclass,
        .IsAbsolutelyRequiredFragmentForAnnotation = True
    }
}
        ...
        Private Function GetAcylDoubleBondSpectrum(ByVal lipid As ILipid, ByVal acylChains As IEnumerable(Of AcylChain), ByVal adduct As AdductIon, ByVal Optional nlMass As Double = 0.0) As IEnumerable(Of SpectrumPeak)
            Return acylChains.SelectMany(Function(acylChain) spectrumGenerator.GetAcylDoubleBondSpectrum(lipid, acylChain, adduct, nlMass - HydrogenMass, 10R))
        End Function
        ...
        Private Function GetAcylLevelSpectrum(ByVal lipid As ILipid, ByVal acylChains As IEnumerable(Of IChain), ByVal adduct As AdductIon) As IEnumerable(Of SpectrumPeak)
            ...
            Dim spectrum = New List(Of SpectrumPeak) From {
    New SpectrumPeak(lipidMass - sn1sn2mass + ProtonMass, 30R, $"[M-Sn1-Sn2-C3H3O2+H]+") With {
        .SpectrumComment = SpectrumComment.acylchain
    },
    New SpectrumPeak(lipidMass - sn3sn4mass + ProtonMass, 30R, $"[M-Sn3-Sn4-C3H3O2+H]+") With {
        .SpectrumComment = SpectrumComment.acylchain
    },
    New SpectrumPeak(lipidMass - sn1sn2mass + ProtonMass - H2O, 30R, $"[M-Sn1-Sn2-C3H3O2-H2O+H]+") With {
        .SpectrumComment = SpectrumComment.acylchain
    },
    New SpectrumPeak(lipidMass - sn3sn4mass + ProtonMass - H2O, 30R, $"[M-Sn3-Sn4-C3H3O2-H2O+H]+") With {
        .SpectrumComment = SpectrumComment.acylchain
    },
    New SpectrumPeak(sn1sn2mass + ProtonMass, 500R, $"[Sn1+Sn2+C3H3O2+H]+") With {
        .SpectrumComment = SpectrumComment.acylchain
    },
    New SpectrumPeak(sn3sn4mass + ProtonMass, 500R, $"[Sn3+Sn4+C3H3O2+H]+") With {
        .SpectrumComment = SpectrumComment.acylchain
    }
    ...
    New SpectrumPeak(chainMass + C3H6O2 + ProtonMass, 100R, $"{acylChain} +C3H6O2") With {
        .SpectrumComment = SpectrumComment.acylchain
    },
    New SpectrumPeak(chainMass + C3H6O2 - OxygenMass + ProtonMass, 50R, $"{acylChain} +C3H6O") With {
        .SpectrumComment = SpectrumComment.acylchain
    },
    New SpectrumPeak(adduct.ConvertToMz(lipid.Mass - chainMass - adduct.AdductIonAccurateMass + ProtonMass), 20R, $"-{acylChain}") With {
        .SpectrumComment = SpectrumComment.acylchain
    },
    New SpectrumPeak(adduct.ConvertToMz(lipid.Mass - chainMass - adduct.AdductIonAccurateMass + ProtonMass - H2O), 20R, $"-{acylChain}- H2O") With {
        .SpectrumComment = SpectrumComment.acylchain
    }
}
        ...
        Private Function GetAcylPositionSpectrum(ByVal lipid As ILipid, ByVal acylChain As IChain, ByVal adduct As AdductIon, ByVal nlMass As Double) As SpectrumPeak()
            Dim lipidMass = lipid.Mass + adduct.AdductIonAccurateMass
            Dim chainMass = acylChain.Mass - HydrogenMass
            Dim spectrum = New List(Of SpectrumPeak) From {
    New SpectrumPeak(lipidMass - chainMass - nlMass - CH2, 20R, "-CH2(Sn1)") With {
