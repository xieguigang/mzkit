...
'Sort the list, then condense for like masses
    ...
    ActiveWorkbook.Worksheets(4).Sort.SortFields.Add2 _
        Key:=Range(Cells(2, 2), Cells(Nmasses + 1, 2)), SortOn:=xlSortOnValues, Order:=xlAscending, _
    ...
    For i = 1 To Npeaks 'presume the peaks are sorted low to high
        If peaks(i, 2) >= peakthresh Then
            ...
            For CS = precursorCS To 2 Step -1
                ...
                thing3 = thing2 / 1000000# * aplus1tol
...
'match peaks
    ...
    For i = 1 To Npeaks
        If Len(peaks(i, 4)) > 0 Then
            ...
            stng2 = ""
            thing1 = peaks(i, 4)
            thing2 = thing1 / 1000000# * matchtol
    ...
    For i = 1 To Npeaks
        If Len(peakmatch(i, 1)) > 0 Then
            ...
            outputwrite(i, 7) = (outputwrite(i, 5) - outputwrite(i, 6)) / outputwrite(i, 6) * 1000000#
            CS = Abs(peaks(i, 3))
