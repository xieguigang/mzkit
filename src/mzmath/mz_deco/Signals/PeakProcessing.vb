...
Namespace MetabolomicsPeakProcessing
    ...
    Public Class ChromatogramTick
        ...
        Public Sub New(time As Double, intensity As Double)
            Me.Time = time
            Me.Intensity = intensity
        End Sub

        Public Overrides Function ToString() As String
            Return $"{Time:F4}s => {Intensity:F2}"
        End Function
    ...
    Public Class ROI
        ...
        ''' </summary>
        ''' <returns></returns>
        Public Property additional As Dictionary(Of String, Double)

        Public Sub New()
            time = New DoubleRange()
            additional = New Dictionary(Of String, Double)()
        End Sub

        Public Overrides Function ToString() As String
            Return $"ROI: rt={rt:F4}s, max={maxInto:F2}, area={peakarea:F2}, SNR={snRatio:F2}"
        End Function
    ...
    Public Class PeakAreaCalculator
        ...
        Private Shared Function GaussianFittingIntegration(roi As ROI,
            ...
            For iter As Integer = 0 To params.GaussianMaxIterations - 1
                ...
                If Math.Abs(detA) < 1e-20 Then Exit For ' 矩阵奇异，退出
